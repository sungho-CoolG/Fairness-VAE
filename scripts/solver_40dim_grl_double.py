...
class Solver(object):
    def __init__(self, args):
        ...
        self.targetcls=classifier(30,2).cuda()
        self.pa_target=classifier(30,2).cuda()
        self.target_pa=classifier(30,2).cuda()
    ...
    def train(self):
        ...
        for i_num in range(self.max_iter-self.global_iter) :
            ...
            for i, (x_true1,x_true2,heavy_makeup, male) in enumerate(self.data_loader):
                ...
                '''
                #weight=torch.tensor([1.0,3.5]).cuda()
                ...
                #t_rev_cls=F.cross_entropy(t_reverse_output,male,weight=weight)
                t_rev_cls=F.cross_entropy(t_reverse_output,male)

               
                vae_loss = vae_recon_loss + vae_kld + self.alpha*self.gamma*vae_tc_loss +self.grl*t_rev_cls+self.grl*rev_cls
                # + self.beta*pa_cls + self.beta*t_pa_cls 
                #+
